...
$theme_data = get_theme_data(ABSPATH . 'wp-content/themes/silverScreen/style.css');
...
function roots_add_rewrites($content) {
	global $wp_rewrite;
	$roots_blank_non_wp_rules = array(
		'css/(.*)'      => 'wp-content/themes/silverScreen/css/$1',
		'js/(.*)'       => 'wp-content/themes/silverScreen/js/$1',
		'images/(.*)'      => 'wp-content/themes/silverScreen/images/$1',
		'plugins/(.*)'  => 'wp-content/plugins/$1'
	);
	$wp_rewrite->non_wp_rules += $roots_blank_non_wp_rules;
}

add_action('admin_init', 'roots_flush_rewrites');

function roots_clean_assets($content) {
    $current_path = '/wp-content/themes/silverScreen';
    $new_path = '';
...
function roots_continue_reading_link() {
	return ' <a href="' . get_permalink() . '" class="button">' . __( 'Read More', 'paramount' ) . '</a>';
}

function roots_auto_excerpt_more($more) {
	return ' &hellip;' . roots_continue_reading_link();
	#str_replace('[...]', 'É<a href="'. get_permalink($post->ID) . '">[+]</a>', $more);
}
